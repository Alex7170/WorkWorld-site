...
router.get("/registration2", passport.authenticate('jwt', {session:false, failureRedirect: "/api/auth/login"}), controller.getSecondRegister)
router.post("/login", urlencodedParser, controller.login)
router.post("/registration1", urlencodedParser, controller.firstRegister)
router.put("/registration2", passport.authenticate('jwt', {session:false, failureRedirect: "/api/auth/login"}),  urlencodedParser,  controller.secondRegister)
